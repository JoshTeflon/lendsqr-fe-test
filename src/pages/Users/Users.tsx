...
import { Button, Card, Input, Popup, Select, StatusPill, LoadingDots } from '../../components/interface'
import { UserInfoItem } from '../../lib/data/getUsersData'
import { HexToRgba, beforeTodayCheck, convertToThousands, formatDate } from '../../lib/utils'
import { UsersOutline, GroupUsersOutline, UsersLoan, UsersSavings, Filter, Options, Eye, UserCancel } from '../../components/icons'
import { useQuery } from '@tanstack/react-query'
import { getUsers } from '../../api/helper/users'
import { Pagination } from '../../components/global'
import { getOrgOptions, getStatusOptions } from '../../lib/data/getSelectOptions'
...
const Users: React.FC = () => {
    ...
    const { isLoading, refetch } = useQuery(['users'], () => getUsers(orgName, userName, email, phoneNumber), {
        onSuccess: ({ data }) => {
            ...
            setTotalLoanUsers(data?.filter((i: UserInfoItem)  => Number(i?.education?.loanRepayment) > 0)?.length)
            setTotalSavingsUsers(data?.length - data?.filter((i: UserInfoItem)  => Number(i?.education?.loanRepayment) > 0)?.length)
        },
        onError: ({ response }) => {
    ...
    const renderUsers = () => {
        ...
        return users?.slice(startIndex, endIndex).map((j: UserInfoItem, index: number) => (
            <tr key={index}>
                ...
                <td>
                    <div className='user-options'>
                        <StatusPill status={beforeTodayCheck(j.lastActiveDate) ? 'Inactive' : 'Active'} />
                        <Button
                        ...
                        >
                            ...
                            <Button
                                ...
                                className='popup-btn'
                            >
                                <UserCancel /><span>Blacklist User</span>
                            </Button>
                            <Button
                                variant='naked'
                                className='popup-btn'
                            >
                                <Eye /><span>Activate User</span>
                            </Button>
                        </Popup>
                    </div>
                </td>
